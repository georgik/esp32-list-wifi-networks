...
use esp_hal::{Blocking, interrupt::software::SoftwareInterruptControl};
use log::{error, info};
...
async fn main(spawner: Spawner) -> ! {
    ...
    let mut i2c_bus = I2c::new(peripherals.I2C0, esp_hal::i2c::master::Config::default())
        .unwrap()
    ...
    if hsync_pulse < 4 {
        error!(
            "HSYNC pulse width is too low: {} pixels, setting to minimum 4 pixels",
            hsync_pulse
        );
    }
    ...
    if hsync_back_porch < 43 {
        error!(
            "HSYNC back porch is too low: {} pixels, setting to minimum 43 pixels",
            hsync_back
        );
    }
    ...
    if hsync_front_porch < 8 {
        error!(
            "HSYNC front porch is too low: {} pixels, setting to minimum 8 pixels",
            hsync_front
        );
    }
    ...
    if vsync_pulse < 4 {
        error!(
            "VSYNC pulse width is too low: {} lines, setting to minimum 4 lines",
            vsync_pulse
        );
    }
    ...
    if vsync_back_porch < 12 {
        error!(
            "VSYNC back porch is too low: {} lines, setting to minimum 12 lines",
            vsync_back
        );
    }
    ...
    if vsync_front_porch < 8 {
        error!(
            "VSYNC front porch is too low: {} lines, setting to minimum 8 lines",
            vsync_front
        );
    }
    ...
    // Initialize Embassy with software interrupt (ESP32-S3 is Xtensa - single timer only)
    ...
    esp_rtos::start_second_core(
        ...
        move || {
            ...
            executor.run(|spawner| {
                spawner
                    .spawn(conway_task(unsafe { PSRAM_BUF_PTR }, unsafe {
                        PSRAM_BUF_LEN
                    }))
                    .ok();
            });
...
// Conway update task running on app core
